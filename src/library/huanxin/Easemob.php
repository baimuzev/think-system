...
namespace BaiMuZe\Admin\library\huanxin;
...
class Easemob
{
    ...
    public function __construct($options = array())
    {
        if (empty($options)) {
            ...
            $options = $config->getConfigs('huanxin', 'huanxin');
        }
...
*/
     ...
     * @param $avatar
     */
    public function editAvatar($username, $avatar)
    {
        $url = $this->url . 'users/' . $username;
        $options = array(
            "avatar" => $avatar
...
*/
    ...
    */
    ...
    /*
        发送文本消息
    */
    function sendText($target_type, $target, $content, $ext, $from = "admin")
    {
    ...
    /*
        发送透传消息
    */
    function sendCmd($target_type, $target, $action, $ext, $from = "admin")
    {
        $url = $this->url . 'messages';
        $body['target_type'] = $target_type;
        $body['target'] = $target;
    ...
    /*
        发图片消息
    */
    function sendImage($filePath, $target_type, $target, $filename, $ext, $from = "admin")
    {
    ...
    /*
        发语音消息
    */
    function sendAudio($filePath, $target_type, $target, $filename, $length, $ext, $from = "admin")
    {
        $result = $this->uploadFile($filePath);
        $uri = $result['uri'];
        $uuid = $result['entities'][0]['uuid'];
    ...
    /*
        发视频消息
    */
    function sendVedio($filePath, $target_type, $target, $filename, $length, $thumb, $thumb_secret, $ext, $from = "admin")
    {
        $result = $this->uploadFile($filePath);
        $uri = $result['uri'];
        $uuid = $result['entities'][0]['uuid'];
    ...
    发文件消息
    */
    function sendFile($filePath, $target_type, $target, $filename, $length, $ext, $from = "admin")
    {
        $result = $this->uploadFile($filePath);
        $uri = $result['uri'];
        $uuid = $result['entities'][0]['uuid'];
    ...
    */
    ...
    /*
       群组设置管理员
   */
    function addGroupAdmin($group_id, $username)
    {
    ...
    /*
     群组移除管理员
    */
    function deleteGroupAdmin($group_id, $username)
    {
    ...
    */
    function deleteGroupMembers($group_id, $usernames)
    {
        ...
        //$body=json_encode($usernames);
        $header = array($this->getToken());
        $result = $this->postCurl($url, '', $header, 'PUT');
        return $result;
    }
    ...
    */
    ...
    /*
        导出聊天记录---分页
    */
    function getChatRecordForPage($ql, $cursor, $limit = 0)
    {
        if (!empty($ql)) {
