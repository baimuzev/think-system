...
use BaiMuZe\Admin\library\Storage;
use BaiMuZe\Admin\utility\Http;
...
class TxcosStorage extends Storage
{
    ...
    {
        // 读取配置文件
        $config = \BaiMuZe\model\Storage::mk()->where('label', 'txcos')->find();
        if (!is_null($config) && !$config['status']) {
            $config = json_decode($config['config']);
            $this->point = $config->REGION;
            $this->bucket = $config->name;
            $this->secretId = $config->ACCESS_KEY;
            $this->secretKey = $config->SECRET_KEY;
            $host = Str::lower($config->https);
            $this->prefix = "//{$host}";
        } else {
    ...
    public function info(string $name, bool $power = false, ?string $attname = null): array
    {
        return $this->has($name, $power) ? [
            ...
            'key' => $name, 'file' => $this->path($name, $power),
        ] : [];
    }
