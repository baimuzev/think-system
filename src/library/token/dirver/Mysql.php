...
namespace BaiMuZe\Admin\library\token\dirver;


use BaiMuZe\Admin\library\token\Driver;
use think\facade\Db;
...
class Mysql extends Driver
{
     ...
     * @return bool
     */
    public function set($token, $user_id, $expire = null, $model = 'admin')
    {
        $expiretime = !is_null($expire) && $expire !== 0 ? time() + $expire : 0;
        $token = $this->getEncryptedToken($token);
        $this->handler->insert(['token' => $token, 'uid' => $user_id, 'createtime' => time(), 'expiretime' => $expiretime, 'type' => $model]);
        return true;
    }
    ...
    {
        $data = $this->get($token);
        return $data && $data['uid'] == $user_id ? true : false;
    }
