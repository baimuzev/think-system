...
use BaiMuZe\Admin\library\ModuleService;
use BaiMuZe\Admin\library\NodeServer;
 ...
 * 组件安装指令
 * Class Publish
 * @package BaiMuZe\Admin\library\Command
 */
class Publish extends Command
{
    ...
    public function configure()
    {
        ...
        $this->setDescription('Publish Plugs and Config Assets for Baimuze');
    }
    ...
    private function copy(string $copy, bool $force = false)
    {
        ...
        File::move($frdir, syspath('config'), 0);

        // 复制静态资料文件
        $frdir = rtrim($copy, '\\/') . DIRECTORY_SEPARATOR . 'public';
        File::move($frdir, syspath('public'), 0);

        // 复制数据库脚本
        $frdir = rtrim($copy, '\\/') . DIRECTORY_SEPARATOR . 'database';
        File::move($frdir, syspath('database/migrations'), 0);
    }
